			...
			<div class="panel">
				<div class="panel-heading">
					<div class="panel-title">
                        <h1>Sub Documents of {{$dt->dt_title}}</h1>
          </div>
				</div><!--.panel-heading-->
				<div class="panel-body">
                <a href="{{URL::to('/admin/addsubdoc')}}/{{$dt->dt_id}}" class="btn btn-primary">Add Sub Document</a>
        <div class="overflow-table">
					<table class="table table-responsive display datatables-basic">
						...
						@if(!empty($docs))
							@foreach($docs as $d)
							<tr>
                            <td id="title{{$d->sd_id}}">{{$d->sd_title}}</td>
								
								<td id="desc{{$d->sd_id}}"><?php echo substr($d->sd_description,0,50); ?></td>
                                <td>
										<a href="{{ URL::to('/admin/enterprisedocs') }}/{{ $d->sd_id }}" lr_id="{{$d->sd_id}}" class="view btn btn-success">View Docs</a>
                            
                                <a href="{{URL::to('/admin/deleteSubDoc')}}/{{$d->sd_id}}" class="btn btn-danger">Delete</a>
								</td>
							</tr>
						@endforeach
						@endif
